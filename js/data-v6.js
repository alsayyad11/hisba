...
import * as remote from './data.remote.js?v=account-type-map-v5-budget-account';
import { getTable, replaceTable, upsertLocal, removeLocal, enqueue, ensureId, applyFilters, decorate, syncUser, bindSync, getQueue, getSyncMeta, hasHydratedTable, markTableHydrated, removeQueueItem, discardQueuedEntity, discardQueuedDelete, markQueueItemFailed, isOffline, purgeAccountsByName } from './offline.js?v=queue-schema-v17';
...
async function save(userId, table, row, creator, payload = row) {
  ...
  if (!isOffline()) {
    try {
      ...
      const failure = new Error(error?.message || 'SYNC_WRITE_FAILED');
      failure.code = error?.code || error?.status || 'SYNC_WRITE_FAILED';
      failure.__syncPending = true;
      throw failure;
    }
  }
...
export async function createAccount(userId, account) { const row = ensureId({ ...account, user_id: userId, created_at: account.created_at || new Date().toISOString() }); discardQueuedDelete(userId, 'accounts', row.id); return save(userId, 'accounts', row, () => remote.createAccount(userId, row), row); }
export async function updateAccount(id, userId, updates) {
  discardQueuedDelete(userId, 'accounts', id);
  return patch(userId, 'accounts', id, updates, () => remote.updateAccount(id, userId, updates));
}
export async function deleteAccount(id, userId) {
  const account = getTable(userId, 'accounts').find(row => row.id === id);
  const transactions = getTable(userId, 'transactions').filter(row => row.account_id === id);
  const result = await remove(userId, 'accounts', id, () => remote.deleteAccount(id, userId));
  transactions.forEach(transaction => removeLocal(userId, 'transactions', transaction.id));
  notifyFinancialDataChanged(userId, ['accounts', 'transactions']);
  return result;
}
export function purgeTestWallets(userId) { return purgeAccountsByName(userId); }
...
const DEFAULT_CATEGORIES = [
  ...
  ['بن', 'Coffee', 'coffee', '#8b5e3c'],
  ['ورك سبيس', 'Workspace', 'workspace', '#7c6cff'],
  ['التزام شخصي', 'Personal Commitment', 'shield', '#9b7cff'],
].map(([name_ar, name, icon, color]) => ({ name_ar, name, icon, color, type: 'expense', is_predefined: true }));

function normalizeCategoryLabel(value) {
  return String(value || '').normalize('NFKC').replace(/[\u064B-\u065F\u0670\u0640]/g, '').replace(/\s+/g, ' ').trim().toLocaleLowerCase();
}

function dedupeCategories(rows, userId) {
  const transactions = getTable(userId, 'transactions');
  const usage = new Map();
  for (const tx of transactions) {
    if (tx?.category_id) usage.set(String(tx.category_id), (usage.get(String(tx.category_id)) || 0) + 1);
  }
  const groups = [];
  for (const cat of rows) {
    const aliases = [normalizeCategoryLabel(cat.name_ar), normalizeCategoryLabel(cat.name)].filter(Boolean);
    const current = groups.find(group => aliases.some(alias => group.aliases.includes(alias)));
    const score = (usage.get(String(cat.id)) || 0) * 1000 + (cat.user_id === userId ? 100 : 0) + (cat.is_predefined ? 1 : 0);
    if (!current) groups.push({ aliases, cat, score });
    else if (score > current.score) { current.aliases = [...new Set([...current.aliases, ...aliases])]; current.cat = cat; current.score = score; }
    else current.aliases = [...new Set([...current.aliases, ...aliases])];
  }
  return groups.map(group => group.cat);
}

export async function getCategories(userId) {
  const rows = await readCached(userId, 'categories', () => remote.getCategories(userId));
  const existing = new Set(rows.flatMap(cat => [String(cat.name || '').trim().toLowerCase(), String(cat.name_ar || '').trim().toLowerCase()].filter(Boolean)));
  const missing = DEFAULT_CATEGORIES.filter(cat => !existing.has(cat.name.toLowerCase()) && !existing.has(cat.name_ar.toLowerCase()));
  let result = rows;
  if (missing.length) {
    const seeded = await Promise.all(missing.map(async cat => {
      try { return await createCategory(userId, cat); }
      catch { return ensureId({ ...cat, user_id: userId }); }
    }));
    result = [...rows, ...seeded];
  }
  return dedupeCategories(result, userId);
}
...
export async function createTransaction(userId, tx) {
  if (tx.account_id) discardQueuedDelete(userId, 'accounts', tx.account_id);
...
export async function updateTransaction(id, userId, updates) {
  ...
  // Editing a transaction to use an account is an explicit reuse of that account.
  // Cancel only a stale queued account-delete operation before the balance write;
  // otherwise the background sync can delete the account immediately after save.
  if (next.account_id) discardQueuedDelete(userId, 'accounts', next.account_id);
...
export async function deleteTransaction(id, userId) {
  ...
  if (!current) return { id, __syncPending: false };

  // A previous offline create/update for this same row may still be in the
  // queue. Let the active sync pass finish, then remove those stale writes
  // before issuing the delete so they cannot resurrect the transaction.
  const activeSync = inFlightSyncs.get(userId);
  if (activeSync) await activeSync.catch(() => null);
  discardQueuedEntity(userId, 'transactions', id);
  removeLocal(userId, 'transactions', id);

  let result;
  if (!isOffline()) {
    try {
      await remote.deleteTransaction(id, userId);
      // A successful remote delete is terminal for this entity. Do not leave
      // any tag or transaction mutation that could recreate it later.
      discardQueuedEntity(userId, 'transactions', id);
      result = { id, __syncPending: false };
    } catch (error) {
      result = { id, __syncPending: true };
      queueAfterFailedWrite(userId, { action: 'delete', table: 'transactions', id }, error);
    }
  } else {
    result = { id, __syncPending: true };
    queueAfterFailedWrite(userId, { action: 'delete', table: 'transactions', id });
  }
...
export async function getBudgetSpending(userId, budgets = [], periodStart, periodEnd, preloadedTransactions = null) {
  ...
  txs.forEach(tx => {
    ...
    const txCategoryId = normalizeId(tx.category_id || tx.category?.id);
    const txAccountId = normalizeId(tx.account_id || tx.account?.id);
    budgets.forEach(budget => {
      const budgetCategoryId = normalizeId(budget.category_id || budget.category?.id);
      const budgetAccountId = normalizeId(budget.account_id || budget.funding_account?.id);
      const sameCategory = !budgetCategoryId || budgetCategoryId === txCategoryId;
      const sameAccount = !budgetAccountId || budgetAccountId === txAccountId;
      if (sameCategory && sameAccount) {
        const key = normalizeId(budget.id);
        if (key) spending[key] = (spending[key] || 0) + amount;
      }
    });
