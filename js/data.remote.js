...
// ── Accounts ───────────────────────────────────────────────
// The web UI has richer labels than the legacy Supabase check constraint.
// Keep the translation at the remote boundary so local/offline data remains
// expressed in the UI vocabulary.
// Supabase uses the same canonical values as the web form and its check constraint.
// Keep the mapping explicit so future UI labels cannot silently change the persisted enum.
const UI_TO_DB_ACCOUNT_TYPE = {
  checking: 'bank',
  savings: 'other',
  cash: 'cash',
  credit: 'card',
...
function toRemoteAccount(account) {
  const type = UI_TO_DB_ACCOUNT_TYPE[account?.type] || account?.type || 'cash';
  return { ...account, type };
}

function fromRemoteAccount(account) {
  if (!account) return account;
  return { ...account, type: DB_TO_UI_ACCOUNT_TYPE[account.type] || account.type || 'cash' };
}
...
export async function getAccounts(userId) {
  ...
  return (data || []).map(fromRemoteAccount);
}

export async function createAccount(userId, account) {
  const { data, error } = await supabase
    .from('accounts')
    .insert({ ...toRemoteAccount(account), user_id: userId })
    .select().single();
  if (error) throw error;
  return fromRemoteAccount(data);
}

export async function updateAccount(id, userId, updates) {
  const { data, error } = await supabase
    .from('accounts')
    .update(toRemoteAccount(updates))
...
export async function deleteAccount(id, userId) {
  // Delete dependent financial rows first so legacy installations without
  // ON DELETE CASCADE can still remove a test wallet safely.
  const { data: transactions, error: transactionReadError } = await supabase
    .from('transactions')
    .select('id')
    .eq('account_id', id)
    .eq('user_id', userId);
...
// ── Budgets ────────────────────────────────────────────────
// The live budgets table intentionally has no user-entered name column.
// Keep the UI vocabulary local, but send only columns supported by Supabase.
function toRemoteBudget(budget = {}) {
  const { name: _name, ...supported } = budget;
  return supported;
}

function fromRemoteBudget(budget) {
  if (!budget) return budget;
  const categoryName = budget.category?.name || budget.category?.name_ar;
  return { ...budget, name: budget.name || categoryName || 'Budget' };
}
...
export async function getBudgets(userId) {
  const { data, error } = await supabase
    ...
    .order('created_at');
  if (error) throw error;
  return (data || []).map(fromRemoteBudget);
}

export async function createBudget(userId, budget) {
  const { data, error } = await supabase
    .from('budgets')
    .insert({ ...toRemoteBudget(budget), user_id: userId })
    .select(`*, category:categories(id,name,name_ar,icon,color)`)
    .single();
  if (error) throw error;
  return fromRemoteBudget(data);
}

export async function updateBudget(id, userId, updates) {
  const { data, error } = await supabase
    .from('budgets')
    .update(toRemoteBudget(updates))
    .eq('id', id).eq('user_id', userId)
    .select(`*, category:categories(id,name,name_ar,icon,color)`)
    .single();
  if (error) throw error;
...
export async function getBudgetSpending(userId, budgets, periodStart, periodEnd) {
  ...
  (data || []).forEach(tx => {
    budgets.forEach(budget => {
      const sameCategory = budget.category_id ? tx.category_id === budget.category_id : true;
      const sameAccount = budget.account_id ? tx.account_id === budget.account_id : true;
      if (sameCategory && sameAccount) {
        const key = String(budget.id);
        spending[key] = (spending[key] || 0) + Number(tx.amount || 0);
      }
    });
  });
  return spending;
...
export async function replaceTransactionTags(transactionId, userId, tagIds) {
  ...
  const { error: deleteError } = await supabase
    .from('transaction_tags').delete().eq('transaction_id', transactionId);
  if (deleteError) throw deleteError;
  if (!ids.length) return [];
  const { data, error } = await supabase
    .from('transaction_tags')
    .insert(ids.map(tag_id => ({ transaction_id: transactionId, tag_id })))
