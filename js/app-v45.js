...
const pageLoaders = {
  ...
  bills:        () => import('./pages/bills-v39.js?v=currency-display-v3'),
  settings:     () => import('./pages/settings-final.js?v=currency-display-v2'),
...
async function boot() {
  ...
  showLoadingOverlay();
  // Never leave a user on an infinite spinner. Network/auth calls are allowed
  // to continue in the background, but the UI must provide a recovery action.
  ...
  // Profile enrichment must never block the offline-first shell. If Supabase
  // is slow or temporarily unavailable, render immediately from auth metadata
  // and let later data refreshes fill the optional profile fields.
  ...
  // Hisba is Egypt-first: only profiles with no saved currency use EGP.
  // Explicit currencies such as USD remain unchanged for existing users.
  if (!currentProfile.currency) currentProfile.currency = 'EGP';
...
function showLoadingOverlay() {
  ...
  body.innerHTML = `
    <div class="loading-overlay">
      <div class="loading-logo"><img class="hisba-logo-image" src="/assets/hisba-logo-transparent-gold-final.png" alt="${brandName}"><span class="hisba-logo-wordmark">${brandName}</span></div>
      <div class="loading-spinner"></div>
      <p class="loading-status">${getLanguage().startsWith('ar') ? 'بنجهّز حسابك…' : 'Preparing your workspace…'}</p>
    </div>`;
}
