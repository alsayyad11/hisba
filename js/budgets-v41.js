/* ============================================================
   HISBA — BUDGETS PAGE
   ============================================================ */
import { t, formatCurrency, formatPercent, validateRequired, validateAmount, getMonthRange, getCurrentMonth, getDateRange, getLanguage, renderIcon, escapeHTML, sanitizeColor } from '../utils.js?v=currency-display-en-v2';
import { getBudgets, createBudget, updateBudget, deleteBudget, getAccounts, getCategories, getBudgetSpending, getTransactions, getMonthClosures, saveMonthClosure } from '../services/data-v6.js?v=budget-funding-account-v2';
...
let budgets = [], accounts = [], categories = [], spending = {}, transactions = [], closures = [];
let editingBudget = null;
let onboardingActive = false;

export async function initBudgets(uid, profile, opts = {}) {
  userId = uid;
  userCurrency = profile?.currency || 'EGP';
...
async function loadData() {
  ...
  const [allBudgets, loadedAccounts, loadedCategories, loadedTransactions, loadedClosures] = await Promise.all([
  ...
  budgets = budgets.map(b => ({ ...b, spent: spending[String(b.id)] || 0 }));
}
...
function budgetCard(b) {
  ...
  const categoryColor = sanitizeColor(b.category?.color, '#3ec3d5');
  const fundingAccount = accounts.find(account => account.id === b.account_id) || b.funding_account;
  const accountLabel = fundingAccount?.name || t('budget_funding_account_optional');

  return `<article class="budget-ledger-card ${statusClass}">
    <div class="budget-ledger-main"><span class="budget-category-icon" style="--budget-category-color:${categoryColor}">${renderIcon(b.category?.icon || 'package', 18)}</span><div><h3>${escapeHTML(b.name)}</h3><p>${escapeHTML(categoryName)}<i aria-hidden="true">·</i>${t('period_' + (b.period === 'weekly' ? 'weekly' : 'monthly'))}<i aria-hidden="true">·</i><span class="budget-funding-account">${escapeHTML(accountLabel)}</span></p></div></div>
...
function buildModal(b) {
  ...
  createModal({
    ...
    content: `
      ...
      <div class="form-group">
        <label class="form-label">${t('budget_funding_account')}</label>
        <select class="form-select" id="b-account">
          <option value="">${t('budget_funding_account_optional')}</option>
          ${accounts.map(account => `<option value="${escapeHTML(account.id)}" ${b?.account_id === account.id ? 'selected' : ''}>${escapeHTML(account.name || t('account_name'))}</option>`).join('')}
        </select>
        <div class="text-caption text-muted">${t('budget_funding_account_hint')}</div>
      </div>
      <div class="form-group">
  ...
  document.getElementById('b-save')?.addEventListener('click', async () => {
    ...
    const account_id = document.getElementById('b-account').value || null;
    ...
    try {
      const payload = { name, amount: parseFloat(amount), period, month_key, week_key, category_id, account_id };
