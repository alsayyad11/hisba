/* ============================================================
   HISBA — SETTINGS PAGE
   ============================================================ */
import { t, setLanguage, getLanguage, getTheme, setTheme, validateRequired, validatePassword, renderIcon } from '../utils.js?v=lang-v5';
...
function renderPage() {
  ...
  document.getElementById('profile-avatar-input')?.addEventListener('change', async e => { const file = e.target.files?.[0]; if (!file) return; const preview = document.getElementById('profile-avatar-preview'); if (preview) { preview.style.backgroundImage = `url('${URL.createObjectURL(file)}')`; preview.style.backgroundSize = 'cover'; preview.style.backgroundPosition = 'center'; preview.style.color = 'transparent'; preview.textContent = ''; } try { const avatarUrl = await uploadProfileAvatar(userId, file); profile.avatar_url = avatarUrl; window.dispatchEvent(new CustomEvent('profileupdated', { detail: { avatar_url: avatarUrl } })); toast.success(getLanguage().startsWith('ar') ? 'اتحفظت الصورة' : 'Photo saved', getLanguage().startsWith('ar') ? 'صورة البروفايل اتحدثت على حسابك.' : 'Your profile photo was updated.'); } catch (err) { const ar = getLanguage().startsWith('ar'); if (err?.localUrl) { if (preview) { preview.style.backgroundImage = `url("${err.localUrl}")`; preview.style.backgroundSize = 'cover'; preview.style.backgroundPosition = 'center'; } profile.avatar_url = err.localUrl; window.dispatchEvent(new CustomEvent('profileupdated', { detail: { avatar_url: err.localUrl } })); toast.warning(err.code === 'OFFLINE' ? (ar ? 'اتحفظت على الجهاز' : 'Saved on this device') : (ar ? 'اتحفظت مؤقتاً' : 'Saved locally for now'), ar ? 'الصورة هتتزامن أول ما الإنترنت يرجع.' : 'It will sync when you are back online.', 7000); } else toast.error(ar ? 'الصورة ما اتحفظتش' : 'Photo was not saved', err?.message || (ar ? 'تعذر حفظ الصورة.' : 'The photo could not be saved.')); } });
