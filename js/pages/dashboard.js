...
function cashflowSeries(openingBalance) { const byDay = new Map(); monthTx.forEach(transaction => { const date = String(transaction.date || '').slice(0, 10); if (!date) return; const move = transaction.type === 'income' ? Number(transaction.amount || 0) : transaction.type === 'expense' ? -Number(transaction.amount || 0) : 0; byDay.set(date, (byDay.get(date) || 0) + move); }); let running = Number(openingBalance || 0); return [...byDay.entries()].sort(([a],[b]) => a.localeCompare(b)).map(([date,move]) => ({ label: date.slice(8), value: running += move })); }
